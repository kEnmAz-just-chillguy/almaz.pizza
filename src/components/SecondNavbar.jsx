...
function SecondNavbar({user}) {
    return (
      <div className=' bg-opacity-80 backdrop- h-full text-black'>
    ...
    <div className="flex flex-wrap items-center justify-between px-4 py-2 text-sm font-medium text-gray-700 max-w-screen-xl mx-auto">
      ...
      <button className="text-blacke font-medium text-sm flex items-center gap-2 hover:text-primary" onClick={()=> {
        if (!user) {
    ...
    <div className="flex items-center justify-between px-4 py-3 max-w-screen-xl mx-auto">
      <a className='flex items-center gap-3 font-bold text-2xl ' href="/">
      <IoDiamond width="36"/>
      <div>AlmazPizza</div>
      </a>
      <Link
